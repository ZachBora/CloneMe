	...
	public void onBlockBreak(BlockBreakEvent event) {
		...
		if(!(event instanceof CloneBlockBreakEvent))
		{
			...
			if(clones != null && clones.size() != 0)
			{
				...
				{
					plugin.schedule(new ScheduledBlockChange(clone,plugin, p, event.getBlock(), changetype.blockbreak, event), 1);
				}
			}
		}
	}
	...
	public void onBlockPlace(BlockPlaceEvent event) {
		...
		if(!(event instanceof CloneBlockPlaceEvent))
		{
			...
			if(clones != null && clones.size() != 0)
			{
				for(Clone clone : clones)
				{
					plugin.schedule(new ScheduledBlockChange(clone,plugin, p, event.getBlock(), changetype.blockplace, event), 1);
				}
			}
		}
	}
