...
public class CMCommand implements CommandExecutor {
	...
	if (l.equalsIgnoreCase("cloneme")) {
	    if (!(s instanceof Player)
		    || plugin.checkPermissions((Player) s, "CloneMe.use")) {
		if (a.length == 0) {
		    s.sendMessage(ChatColor.BLUE + CloneMe.NAME + " v"
			    + CloneMe.VERSION + "_" + CloneMe.SUBVERSION
			    + ChatColor.WHITE
		    ...
		    if (a.length == 1) {
			s.sendMessage(ChatColor.BLUE + CloneMe.NAME + " v"
				+ CloneMe.VERSION + "_" + CloneMe.VERSION + " "
				+ ChatColor.RED + "/cloneme add [params]");
