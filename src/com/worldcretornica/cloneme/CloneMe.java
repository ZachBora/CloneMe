...
public class CloneMe extends JavaPlugin {
	...
	public void onDisable() {
		...
		for(Set<Clone> clones : clonelist.values())
		{
			for(Clone clone : clones)
			{
				if(npcManager != null)
				{
					Packet29DestroyEntity p29 = new Packet29DestroyEntity(clone.npc.getEntityId());
	...
	public void onEnable() {
		...
		if (getServer().getPluginManager().isPluginEnabled("NPCCreatures"))
		{
			...
			this.logger.info("[" + pdfdescription + "] NPCCreatures v" + npccreature.getDescription().getVersion() + " found!");
			
			npccreature = null;
		}else{
			npcManager = null;
			this.logger.info("[" + pdfdescription + "] Could not find NPCCreatures, clones will not be visible!");
		}
	...
	public boolean onCommand(CommandSender s, Command c, String l, String[] a)
	{
		...
		{
			if(!(s instanceof Player) || this.checkPermissions((Player) s, "CloneMe.use"))
			{
				if (a.length == 0)
				{
					s.sendMessage(ChatColor.BLUE + pdfdescription + " v" + pdfversion + ChatColor.WHITE + " [] means optional, <> means obligated");
				...
				}else if(a[0].toString().equalsIgnoreCase("save"))
				{
					if(!(s instanceof Player))
					{
						s.sendMessage("[" + pdfdescription + "] Only players can save templates!");
						return true;
					}
				...
				}else if(a[0].toString().equalsIgnoreCase("load"))
				{
					if(!(s instanceof Player))
					{
						s.sendMessage("[" + pdfdescription + "] Only players can load templates!");
						return true;
					}
				...
				}else if(a[0].toString().equalsIgnoreCase("ready"))
				{
					if(!(s instanceof Player))
					{
						s.sendMessage("[" + pdfdescription + "] Only players can be ready!");
						return true;
					}
				...
				}else if(a[0].toString().equalsIgnoreCase("pause"))
				{
					if(!(s instanceof Player))
					{
						s.sendMessage("[" + pdfdescription + "] Only players can pause!");
						return true;
					}
				...
				}else if(a[0].toString().equalsIgnoreCase("add"))
				{
					if(!(s instanceof Player))
					{
						s.sendMessage("[" + pdfdescription + "] Only players can have clones!");
						return true;
					}
					
					if(a.length == 1)
					{
						s.sendMessage(ChatColor.BLUE + pdfdescription + " v" + pdfversion + " " + ChatColor.RED + "/cloneme add [params]");
						...
						for(String arg : a)
						{
							if(arg.toLowerCase().contains("x:"))
							{
								...
								{
									s.sendMessage(ChatColor.RED + "[" + pdfdescription + "] Error : Argument 'x:' need to be followed by a whole number! Ex: 'x:5'");
									return true;
								}
							}else if(arg.toLowerCase().contains("n:"))
							{
								...
								if(name.length() == 0)
								{
									s.sendMessage(ChatColor.RED + "[" + pdfdescription + "] Error : Argument 'n:' need to be followed by a string! Ex: 'n:Bob'");
								}
							}else if(arg.toLowerCase().contains("y:"))
							{
								...
								{
									s.sendMessage(ChatColor.RED + "[" + pdfdescription + "] Error : Argument 'y:' need to be followed by a whole number! Ex: 'y:5'");
									return true;
								}
							}else if(arg.toLowerCase().contains("z:"))
							{
								...
								{
									s.sendMessage(ChatColor.RED + "[" + pdfdescription + "] Error : Argument 'z:' need to be followed by a whole number! Ex: 'z:5'");
									return true;
								}
							}else if(arg.toLowerCase().contains("m:"))
							{
								...
								{
									s.sendMessage(ChatColor.RED + "[" + pdfdescription + "] Error : Argument 'mirror:' need to be a valid direction! Ex: 'mirror:north'");
									return true;
								}
							}else if(arg.toLowerCase().contains("r:"))
							{
								...
								{
									s.sendMessage(ChatColor.RED + "[" + pdfdescription + "] Error : Argument 'r:' need to be followed by a multiple of 90! Ex: 'r:180'");
									return true;
								}
							}
						}
						...
						if(nbparam > 0)
						{
							AddClone((Player) s, xpos, ypos, zpos, rotation, dir, name);
							
							s.sendMessage("Clone added!");
						}else{
							s.sendMessage(ChatColor.RED + "[" + pdfdescription + "] Error : You must at least put one of these: r: x: y: z: m: ");
							return true;
						}
					}
				}else if(a[0].toString().equalsIgnoreCase("modify"))
				{
					if(!(s instanceof Player))
					{
						s.sendMessage("[" + pdfdescription + "] Only players can modify clones!");
						return true;
					}
					
					if(a.length == 1)
					{
						s.sendMessage(ChatColor.BLUE + pdfdescription + " v" + pdfversion + " " + ChatColor.RED + "/cloneme modify <id> [params]");
					...
					if(pl == null && opl == null)
					{
						...
						if (clonelist.containsKey(cloner))
						{
							if(npcManager != null)
							{
								...
								{
									Packet20NamedEntitySpawn p20 = clone.packetMaker(ChatColor.GREEN + clone.name);
							        Packet29DestroyEntity p29 = new Packet29DestroyEntity(clone.npc.getEntityId());
	...
	public void AddClone(Player s, long xpos, long ypos, long zpos, int rotation, direction dir, String name)
	{
	...
	/*
	public direction getPlayerFaceDirection(Player p)
	{
